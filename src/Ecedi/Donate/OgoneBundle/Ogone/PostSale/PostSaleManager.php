        ...
        try {
            ...
            switch ($payment->getStatus()) {
              ...
              case Payment::STATUS_CANCELED:
                $this->container->get('event_dispatcher')->dispatch(DonateEvents::PAYMENT_FAILED, new PaymentCanceledEvent($payment));
                break;
              case Payment::STATUS_PAYED:
                $this->container->get('event_dispatcher')->dispatch(DonateEvents::PAYMENT_FAILED, new PaymentCompletedEvent($payment));
                break;
              case Payment::STATUS_DENIED:
                $this->container->get('event_dispatcher')->dispatch(DonateEvents::PAYMENT_FAILED, new PaymentDeniedEvent($payment));
                break;
              case  Payment::STATUS_AUTHORIZED:
                $this->container->get('event_dispatcher')->dispatch(DonateEvents::PAYMENT_FAILED, new PaymentAuthorizedEvent($payment));
                break;
